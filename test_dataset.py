...
ANNOTATIONS_FILE = "C:\\Users\\vedan\\Downloads\\ml-stuttering-events-dataset\\SEP-28k_labels_test.csv"
AUDIO_DIR = "C:\\Users\\vedan\\Downloads\\ml-stuttering-events-dataset\\clip\\clips"
SAMPLE_RATE = 16000
NUM_SAMPLES = SAMPLE_RATE*3

class StutteringTestDataset(Dataset):
    def __init__(
        self, 
        annotations_file, 
        audio_dir, 
...
if __name__ == "__main__":
    ...
    std = StutteringTestDataset(ANNOTATIONS_FILE, AUDIO_DIR, mel_spectrogram, SAMPLE_RATE, NUM_SAMPLES, device)
    print(std)
    print(f"There are {len(std)} samples in the dataset.")
    for i in range(10):
        signal, label = std[i]
        print(label)
    # std._get_audio_sample_path(2)
    # std._get_audio_sample_label(2)
    # print("--------------")
    # std._get_audio_sample_path(3)
    # std._get_audio_sample_label(3)
    # print(std._get_audio_sample_label(2))
