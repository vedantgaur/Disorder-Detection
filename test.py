...
from test_dataset import StutteringTestDataset, AUDIO_DIR, ANNOTATIONS_FILE, SAMPLE_RATE, NUM_SAMPLES
...
if __name__ == "__main__":
    ...
    std_len = len(std)
    std_arr = np.zeros(std_len)


    # Get a sample from the urban sound dataset for inference
    for i in range(len(std)):
        input, target = std[i][0], std[i][1] # [batch size, num_channels, fr, time]
        input.unsqueeze_(0)
